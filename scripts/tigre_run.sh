#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════
# tigre_run.sh — Full benchmark suite for RTX 5060 Ti (16GB VRAM)
#
# Adapted from jetstream_run.sh for the tigre Linux system.
...
#   ./scripts/tigre_run.sh
#
# What it does:
#   1. Verifies prerequisites (CUDA 12.8, cmake, python3, git)
#   2. Builds both C++ projects for RTX 5060 Ti (Blackwell sm_120)
...
# RTX 5060 Ti = Blackwell sm_120, 16GB VRAM, CUDA toolkit 12.8 (cu128)
CUDA_ARCH="120"
TIGRE_CUDA_HOME="${TIGRE_CUDA_HOME:-/usr/local/cuda-12.8}"
TORCH_INDEX="https://download.pytorch.org/whl/cu128"

# Benchmark settings (tuned for 16GB VRAM)
BENCH_STEPS=100
BENCH_WARMUP=5
BENCH_BATCH=8
BENCH_SEQ=256

# 350M — smaller batch than 125M if memory is tight
BENCH_350_BATCH=4
BENCH_350_SEQ=256
...
setup_tigre_cuda() {
    if [ ! -x "${TIGRE_CUDA_HOME}/bin/nvcc" ]; then
        err "CUDA 12.8 not found at ${TIGRE_CUDA_HOME}. Install: sudo apt install -y cuda-toolkit-12-8"
    fi
    export CUDA_HOME="$TIGRE_CUDA_HOME"
...
check_prerequisites() {
    ...
    setup_tigre_cuda
    log "nvcc: $("${CUDA_HOME}/bin/nvcc" --version | grep release)"
    ...
    if command -v nvidia-smi >/dev/null; then
        ...
        case "$GPU_NAME" in
            *5060*) ;;
            *) warn "Expected RTX 5060 Ti; found: $GPU_NAME" ;;
        esac

        if [ "${GPU_MEM:-0}" -lt 14000 ]; then
            warn "GPU has less than 14GB VRAM — 350M model may OOM"
        fi
    fi
...
build_llm_cpp() {
    ...
    if ! python3 -c "import torch" 2>/dev/null; then
        err "PyTorch not installed. Run: pip install torch --index-url ${TORCH_INDEX}"
    fi

    log "Building with tigre_build.sh (sm_${CUDA_ARCH}, CUDA 12.8)..."
    CMAKE_CUDA_ARCHITECTURES="$CUDA_ARCH" "$SCRIPT_DIR/tigre_build.sh"
...
setup_python_env() {
    ...
    if [ -d "$VENV_DIR" ] && "$VENV_DIR/bin/python" -c "import torch" 2>/dev/null; then
        ...
        "$VENV_DIR/bin/pip" install torch --index-url "$TORCH_INDEX" -q \
            || err "PyTorch cu128 install failed"
...
run_training_benchmark() {
    ...
    # ── 350M benchmark (llm-cpp) ──
    log "Benchmarking 350M model (llm-cpp, batch=${BENCH_350_BATCH})..."
...
═══════════════════════════════════════════════════════════════
  ...
  Target: NVIDIA RTX 5060 Ti (16GB VRAM, sm_120, CUDA 12.8)
═══════════════════════════════════════════════════════════════
HEADER
...
main() {
    echo -e "${BOLD}"
    echo "  ╔═══════════════════════════════════════════════════╗"
    echo "  ║  OLMo-corecpp: Full Benchmark Suite (5060 Ti)  ║"
